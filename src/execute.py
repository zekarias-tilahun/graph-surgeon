...
class SelfExec:
    ...
    def __init_data(self):
        ...
        if utils.VAL_LOADER in compiled_dataset:
            self._val_loader = compiled_dataset[utils.VAL_LOADER]
        if utils.TEST_LOADER in compiled_dataset:
            self._test_loader = compiled_dataset[utils.TEST_LOADER]
    ...
    def __reset_model(self):
        ...
        encoder_type = "sage" if args.loader in {"cluster", "subgraph"} else "gcn"
        net = Encoder(
    ...
    def pick_loader(self, which="val"):
        assert which in {"train", "val", "test", "subgraph"}
        if which == "train":
            return self._loader
        elif which == "val":
            assert hasattr(self, "_val_loader")
            return self._val_loader
        elif which == "test":
            assert hasattr(self, "_test_loader")
            return self._test_loader
        elif which == "subgraph":
            assert hasattr(self, "_subgraph_loader")
            return self._subgraph_loader
